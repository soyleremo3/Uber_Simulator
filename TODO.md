...
- [x] **32.** Nav ribbon bina içinden geçme bug'ı. `RouteManager` derinlemesine analiz + yeniden yazıldı: (1) `startNode==endNode` iken artık düz çizgi değil paylaşılan node üzerinden geçiyor; (2) graph yolu yoksa 4 farklı start/end çifti denenir (biased + nearest); (3) absürt sapma (>3×+50m) reddedilir; (4) `EnumerateNeighbors` simetrik adjacency haritası (tek yönlü linkler çift yönlü); (5) `FindEndNode` yanlış taraftaki node'u seçmiyor; (6) `AppendTrimmedGraphPath` baştaki geri node dizisini + sondaki aşırı node'u kırpar; (7) ground-snap no-hit fallback `lastGroundY` (0'a çakmıyor); (8) Dijkstra alloc'suz (her frame 3 koleksiyon yerine reuse). Play test: 43m alım öne dönük → 4 nokta hepsi ileri; 15m arkada alım → temiz düz çizgi. ⚠️ Sahne verisi: her pickup/delivery yakınında bağlı waypoint olduğunu editörde doğrula.
